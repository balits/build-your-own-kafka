...
impl codec::Decoder for KafkaCodec {
    ...
    fn decode(&mut self, src: &mut bytes::BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        info!("Decoded: message_size {message_size}");
        ...
        info!("Decoded request_api_key {request_api_key}");
        let request_api_version = src.get_i16();
        info!("Decoded: request_api_version {request_api_version}");
        let correlation_id = src.get_i32();
        info!("Decoded: correlation_id {correlation_id}");
        ...
        src.advance(body_size);
        let body = Bytes::copy_from_slice(&src[..body_size]);
...
impl codec::Encoder<KafkaResponse> for KafkaCodec {
    ...
    fn encode(&mut self, item: KafkaResponse, dst: &mut bytes::BytesMut) -> Result<(), Self::Error> {
        ensure!(item.message_size > 0, "KafkaRequest had negative message size");
        ensure!((item.message_size as usize) < MAX_BODY_SIZE, "KafkaRequest's message size is bigger than allowed {MAX_BODY_SIZE}");
        ensure!(item.message_size as usize == item.body.len(),
